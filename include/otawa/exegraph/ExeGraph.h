...
class ExeGraph  {
	...
	public:
		...
		inline N * entryNode(void)
			{return _entry_node;}
		void build(FrameWork *fw,  Microprocessor<N> * microprocessor, ExeSequence<N> *sequence);
...
template <class N>
void ExeGraph<N>::build(FrameWork *fw, Microprocessor<N> * microprocessor, ExeSequence<N> *sequence) {
	_sequence = sequence;
  ...
  for (PipelineIterator stage(microprocessor) ; stage ; stage++) {
   ...
   for (InstIterator inst(sequence) ; inst ; inst++) {
    inst->deleteNodes();
    nb_inst++;
    if (inst->basicBlock()->number() != bb_num) {
    	bb_num = inst->basicBlock()->number();
    	cout << "b" << bb_num  << "-";
    }   
  }
