 ...
 *
 *	src/odisplay/BCGDrawer.cpp -- display::BCGDrawer class implementation.
 */
...
void BCGDrawer::make(){
	...
	for(BCG::NodeIterator bb(_bcg); bb; bb++){
	...
	for(BCG::NodeIterator bb(_bcg); bb; bb++){
		...
		for(BCG::Successor succ(bb); succ; succ++){
			BCGEdge* edge = succ.edge();
